...
from frappe.utils import cint, cstr, nowdate, money_in_words, date_diff, add_months, getdate, flt, get_datetime,add_days
...
class ProcessRentforCommerital(Document):
	pass

@frappe.whitelist()	
def get_tenent(property_id,from_period,to_period):
	return frappe.db.sql("select tenant from `tabRent Contract` where docstatus=1 and property='%s' and contract_start_date<='%s' and contract_end_date>='%s' order by contract_end_date desc limit 1"%(property_id,from_period,to_period))
		
@frappe.whitelist()	
def get_rent(property_id,from_period,to_period):
	rent=frappe.db.sql("select rent from `tabRent Contract` where docstatus=1 and property='%s' and contract_start_date<='%s' and contract_end_date>='%s' order by contract_end_date desc limit 1"%(property_id,from_period,to_period))
	if rent:
		difference = relativedelta.relativedelta(get_datetime(add_days(to_period,1)), get_datetime(from_period))
		day_rent=rent[0][0]/monthrange(getdate(to_period).year,getdate(to_period).month)[1]*difference.days
		return rent[0][0]*(difference.years)*12+rent[0][0]*(difference.months)+day_rent,get_tenent(property_id,from_period,to_period)

@frappe.whitelist()	
def create_rent_receipts(from_period=None,to_period=None,building=None,property_id=None,tenant=None,rent=None,rent_amount_in_words=None):
	"""
	...
	for record in contract_list:
		amt=flt(record.get('final_rent_amount'))*(flt(rent)/flt(record.get('rent')))
		...
		if not exist:
			ss = frappe.get_doc({
							...
							"rent_amount":rent,
							"rent_amount_in_words":rent_amount_in_words,
							"final_rent_amount":amt,
...
def tenant_query(doctype, txt, searchfield, start, page_len, filters):
	...
	tenant = frappe.db.sql("""select distinct tenant from `tabRent Contract`
		where docstatus=1 and contract_start_date<=CURDATE() and contract_end_date>=CURDATE() and %s"""%(" and ".join(cond for cond in condition)))
	return tenant
